  ...
  return <aside className="sidebar">
    ...
    <nav className="side-nav" aria-label="Primary navigation">
      {nav.map((item) => {
        ...
        return <Link className={`nav-item ${active ? 'nav-item-active' : ''}`} href={item.href} key={item.href}><Icon />{item.label}{item.href === '/tickets' && <span className="nav-count">2</span>}</Link>
      })}
    </nav>
    <div className="sidebar-bottom"><Link className="nav-item" href="/settings"><Settings />Settings</Link><div className="profile"><div className="avatar">JD</div><div><strong>Jordan Davis</strong><span>jordan@email.com</span></div><MoreHorizontal /></div></div>
  </aside>
}
...
function Overview() {
  return <><Header title="Good morning, Jordan" /><main className="content">
    <section className="hero-ticket"><div><div className="eyebrow light">YOUR NEXT EVENT</div><h2>Future Sound</h2><p className="hero-meta"><CalendarDays /> Saturday, October 18, 2025 <span>•</span> Brooklyn Mirage</p><div className="hero-actions"><Link className="button button-light" href="/tickets/future-sound"><QrCode />View ticket</Link><button className="button button-ghost"><Download />Add to wallet</button></div></div><div className="hero-date"><strong>18</strong><span>OCT<br />2025</span></div></section>
    <div className="section-heading"><div><div className="eyebrow">YOUR TICKETS</div><h2>Ready when you are</h2></div><Link href="/tickets" className="text-link">View all <ArrowUpRight /></Link></div>
    <section className="ticket-grid"><TicketCard event={events[0]} featured /><TicketCard event={events[1]} /></section>
    <section className="dashboard-grid"><div className="panel"><div className="panel-heading"><div><div className="eyebrow">UPCOMING EVENTS</div><h2>Find your next thing</h2></div><Link href="/rsvps" className="text-link">Browse events <ArrowUpRight /></Link></div><div className="event-list">{events.map((event) => <Link href={`/tickets/${event.id}`} className="event-row" key={event.id}><EventArtwork event={event} small /><div className="event-copy"><strong>{event.title}</strong><span>{event.date} · {event.location}</span></div><span className="event-tag">{event.tag}</span><ChevronRight /></Link>)}</div></div><div className="panel activity"><div className="panel-heading"><div><div className="eyebrow">RECENT ACTIVITY</div><h2>What&apos;s happening</h2></div><button className="plain-button" aria-label="More activity"><MoreHorizontal /></button></div><div className="activity-item"><div className="activity-icon"><Check /></div><div><strong>Ticket added to wallet</strong><span>Future Sound · Just now</span></div></div><div className="activity-item"><div className="activity-icon"><CreditCard /></div><div><strong>Order #NS-10482 confirmed</strong><span>Future Sound · Sep 02</span></div></div><div className="activity-item"><div className="activity-icon"><Users /></div><div><strong>RSVP accepted</strong><span>New York Design Week · Aug 28</span></div></div></div></section>
  </main></>
}

function TicketCard({ event, featured = false }: { event: typeof events[number]; featured?: boolean }) {
  return <Link href={`/tickets/${event.id}`} className={`ticket-card ${featured ? 'ticket-featured' : ''}`}><EventArtwork event={event} /><div className="ticket-info"><Status>{featured ? 'Confirmed' : 'Going'}</Status><h3>{event.title}</h3><p>{event.date}<br />{event.location}</p></div><QrCode className="ticket-qr" /></Link>
}
