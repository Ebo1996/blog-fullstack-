...
export const createTicketTypeSchema = z
  .object({
    ...
    price: z
      ...
      .max(100_000_00, 'Price is too high'), // max $100,000 in cents
    currency: z.string().length(3, 'Currency must be 3 characters').default('USD'),
    quantity: z.number().int().positive('Quantity must be positive').max(100000),
    sales_start_at: z.string().datetime({ offset: true }).optional().nullable(),
    sales_end_at: z.string().datetime({ offset: true }).optional().nullable(),
  })
  .refine(
    (data) => {
